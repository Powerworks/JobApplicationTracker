...
Feature 003 (HTTP API, Fastify) implemented 2026-08-18 via the same chain: all 9 endpoints (7 commands, the active-pipeline overview, and an on-demand ghosting-check trigger) reachable over HTTP, each route co-located inside its owning slice/read-model/reactor with no shared `routes.ts`. `POST /applications` generates the application ID; `crypto.randomUUID()` is used, no new ID-generation dependency. Error mapping is a pure translation layer over the existing decider guards (400 malformed input, 404 unknown application, 409 guard violation) — no business logic changed. See `specs/003-application-http-api/`.

Feature 004 (frontend) implemented 2026-08-19 via the same chain: a vanilla-JS, framework-free, single-page UI (hash routing, no build step) served statically by the existing Fastify server via `@fastify/static`. Covers all three user stories — submit/progress an application, the active overview, the ghosting-check trigger. Surfaced and closed a real gap during planning: feature 003 had no way to fetch a single (especially closed) application, so a new `GET /applications/:applicationId` endpoint (its own read-model slice) was added rather than worked around client-side. Also fixed a latent bug found via that new endpoint: `evolve()` never updated `offer.decision` to `Accepted`/`Declined` on those events — only `status` changed; fixed in `src/domain/state.ts`. See `specs/004-application-tracker-frontend/`. 83 passing tests total (`npm test`); run `npm run start` and open `http://localhost:5000`.

Feature 005 (Postgres event store) implemented 2026-08-19 via the same chain: `src/store/event-store.ts` swapped from `getInMemoryEventStore` to `@event-driven-io/emmett-postgresql`'s `getPostgreSQLEventStore`, connected via `DATABASE_URL` and migrated explicitly at server startup (fails fast and loudly if unreachable). The old process-local `application-registry.ts` (never survived a restart, and would have been actively wrong once data did) is replaced by a small persisted `applications` index table (`src/store/application-index.ts`). No decider, route, or frontend contract changes — a pure storage-layer swap, verified by an automated "second app instance, same database" durability test plus a real manual server-restart walkthrough (both pass). Local dev uses `docker-compose.yml`; automated tests spin up their own ephemeral Postgres via `@testcontainers/postgresql` (switched from the originally-planned `@event-driven-io/emmett-testcontainers` after finding and reporting a packaging bug in it — [event-driven-io/emmett#384](https://github.com/event-driven-io/emmett/issues/384)). See `specs/005-postgres-event-store/`. 90 passing tests total (`npm test`, now requires Docker).

ADR 1's "Postgres once the logic is solid" is now fulfilled — see `docs/ADRs.md`.

Not yet done: deploying the tracker anywhere (e.g. GCP) — deliberately the next, separate step now that persistence is real. The tracker is usable end-to-end locally without any direct HTTP calls and without losing data on restart.
