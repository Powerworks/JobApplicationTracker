...
describe("POST /applications/:applicationId/offer/decline", () => {
  it("declines a pending offer (200)", async () => {
    const app = await buildApp();
    const applicationId = await submitScheduleOfferReady(app);

    const response = await app.inject({
      method: "POST",
      url: `/applications/${applicationId}/offer/decline`,
    });
  ...
  it("rejects an unknown applicationId (404)", async () => {
    ...
    const response = await app.inject({
      method: "POST",
      url: "/applications/does-not-exist/offer/decline",
    });

    expect(response.statusCode).toBe(404);
  });

  it("rejects declining twice (409)", async () => {
    const app = await buildApp();
    const applicationId = await submitScheduleOfferReady(app);
    await app.inject({ method: "POST", url: `/applications/${applicationId}/offer/decline` });
